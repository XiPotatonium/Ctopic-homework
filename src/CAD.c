...
EllipseT SelectNearestNodeE(linkedlistADT list, double mx,
                            double my); /*选择靠(mx,my)最近的结点*/
...
void InsertCharToString(string str, int pos,
                        char c); /*将字符c插入到字符串str的pos位置*/
void DeleteCharFromString(string str, int pos); /*删除字符串str的pos位置字符*/
void DrawCursor(string str, int curPos, double startx,
                double starty); /*显示文本光标*/

void PickNearestNode(linkedlistADT list[], double mx,
                     double my); /*选择靠(mx,my)最近结点*/
            ...
            switch (key) {
                ...
                case VK_F4: /*F4: 随机生成空的文本对象*/
                    ...
                    startTimer(CURSOR_BLINK,
                               timerseconds); /*光标闪烁定时器触发*/
                    isCursorBlink = TRUE;     /*设置光标闪烁标志*/
                    break;
                case VK_F5: /*编辑已有文本行*/
                    if (!isSelected || curList != STRING)
                        break;         /*当前不处于文本行编辑状态*/
                    ...
                    strcpy(
                        textbuf,
                        tptr->text); /*将当前文本字符串拷贝到临时文本缓冲区以备编辑*/
                    DeleteNode(list[STRING], curText,
                               TextEqual); /*从当前文本链表中删除该文本对象*/
                    SetPenColor(tptr->color); /*设置选择文本的颜色*/
                    DrawCursor(textbuf, tptr->curPos, tptr->x,
                               tptr->y); /*显示当前文本光标*/
                ...
                case VK_RIGHT:          /*右移键*/
                    ...
                    DrawCursor(textbuf, tptr->curPos, tptr->x,
                               tptr->y); /*擦除光标*/
                    if (tptr->curPos < strlen(textbuf))
                        tptr->curPos++; /*右移光标一个字符位置*/
                    SetEraseMode(FALSE);
                    MovePen(tptr->x, tptr->y);
                    DrawTextString(textbuf); /*重新显示当前文本*/
                    DrawCursor(textbuf, tptr->curPos, tptr->x,
...
void CharEventProcess(char c) {
    ...
    switch (c) {
        ...
        default:
            ...
            DrawCursor(textbuf, tptr->curPos, tptr->x,
                       tptr->y); /*擦除当前光标*/
            InsertCharToString(textbuf, tptr->curPos,
                               c); /*将当前字符插入到光标位置*/
            SetEraseMode(FALSE);
            MovePen(tptr->x, tptr->y);
            DrawTextString(textbuf);
...
void MouseEventProcess(int x, int y, int button, int event) {
    ...
    switch (event) {
        ...
        case MOUSEMOVE:
            if (isSelected &&
                (isMove || isChangeSize)) { /*选中状态下，移动或缩放*/
                SetEraseMode(TRUE);
                TraverseAllList();
                dx = mx - omx;
                dy = my - omy;
                if (isMove) { /*移动*/
                    ...
                    switch (curList) {
                        case LINE:
                            ...
                            break;
                        case RECT:
                            if (fabs(omx - curRect->x1) <
                                fabs(omx - curRect->x2)) {
                                curRect->x1 += dx;
                            } else {
                                curRect->x2 += dx;
                            }
                            if (fabs(omy - curRect->y1) <
                                fabs(omy - curRect->y2)) {
                                curRect->y1 += dy;
                            } else {
                                curRect->y2 += dy;
...
bool TextEqual(void *text1, void *text2) {
    ...
    return StringEqual(tptr1->text, tptr2->text) && tptr1->x == tptr2->x &&
           tptr1->y == tptr2->y;
}
